...
test('updates center from location when location changes', () => {
  ...
  // center should update from location coords
  // You might need to expose center state for testing or verify via map props or behavior
});
...
test('loadMarkers calls getWeatherAtLocation and getRiskFromWeather and sets markers', async () => {
  ...
  // Trigger onLoad manually if needed or test loadMarkers directly (if exported)

  // Wait for markers state update - check if actions called expected number of times
  await waitFor(() => {
...
test('handleRefresh updates center and calls loadMarkers', async () => {
  ...
  // Expect center and markers updated accordingly (you might spy on loadMarkers or check effects)
});
//render tests

//initial state + effects

//loadmarkers behaviour
